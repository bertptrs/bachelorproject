...
\section{Materializing implementations}

Due to time constraints, it was not possible to create a program that could materialize the \emph{forelem} implementation of Push-Lift (see appendix \ref{app:pushlift}) into a compilable or runnable algorithm, as described in. %TODO reference
Instead, we chose to materialize the algorithm by hand and implement some of the variations that a materializer may have produced. For the variations, we chose to vary the data structure in memory. This way, we constructed %TODO
