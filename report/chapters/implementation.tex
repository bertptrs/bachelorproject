...
It then makes sense to consider two sets of tuples. One of edges $E$ containing tuples $(u, v, c)$ with the edge source, sink and remaining capacity, and one of nodes $V$ containing tuples $(u, h, e)$, node, height, and excess. The last has one record per $u$, and is (for integer $u$) probably better off being an array or a map. For the pureness of the implementation however, we will consider it a set of tuples. From this startpoint we will generate the implementation.
